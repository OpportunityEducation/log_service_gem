...
describe LogService::Client do
  ...
  def delete_url(event_collection, filter_params=nil)
    "#{api_url}/api/projects/#{project_id}/events/#{event_collection}#{filter_params ? "?filters=#{CGI.escape(MultiJson.encode(filter_params[:filters]))}" : ""}"
  end
  ...
  describe '#event_collections' do
    let(:events_url) { "#{api_url}/api/projects/#{project_id}/events" }
  ...
  describe '#event_collection' do
    let(:event_collection) { "foodstuffs" }
    let(:events_url) { "#{api_url}/api/projects/#{project_id}/events/#{event_collection}" }
  ...
  describe '#project_info' do
    let(:project_url) { "#{api_url}/api/projects/#{project_id}" }
