...
describe LogService::Client::PublishingMethods do
  ...
  describe "publish" do
    it "should post using the collection and properties" do
      stub_log_service_post(api_event_collection_resource_url(api_url, collection), 201, "")
      client.publish(collection, event_properties)
      expect_log_service_post(api_event_collection_resource_url(api_url, collection), event_properties, "sync", write_key)
    end
    ...
    it "should url encode the event collection" do
      stub_log_service_post(api_event_collection_resource_url(api_url, "User%20posts.new%20)(*%26%5E%25%40!)%3A%3A%2520%2520"), 201, "")
      client.publish("User posts.new )(*&^%@!)::%20%20", event_properties)
      expect_log_service_post(api_event_collection_resource_url(api_url, "User%20posts.new%20)(*%26%5E%25%40!)%3A%3A%2520%2520"), event_properties, "sync", write_key)
    end
    ...
    context "when using proxy" do
      let(:client) do
        LogService::Client.new(:project_id => project_id,
                         :write_key => write_key,
                         :api_url => api_url,
  ...
  describe "publish_batch" do
    ...
    it "should publish a batch of events" do
      stub_log_service_post(api_event_resource_url(api_url), 201, "")
      client.publish_batch(events)
      expect_log_service_post(
  ...
  describe "publish_async" do
    ...
    unless defined?(JRUBY_VERSION)
      it "should require a running event loop" do
        ...
        }.to raise_error(LogService::Error)
      end
      ...
      it "should url encode the event collection" do
        stub_log_service_post(api_event_collection_resource_url(api_url, 'User%20posts.new%20)(*%26%5E%25%40!)%3A%3A%2520%2520'), 201, api_success)
        EM.run {
          client.publish_async('User posts.new )(*&^%@!)::%20%20', event_properties).callback {
            begin
              expect_log_service_post(api_event_collection_resource_url(api_url, 'User%20posts.new%20)(*%26%5E%25%40!)%3A%3A%2520%2520'), event_properties, "async", write_key)
            ensure
              EM.stop
            end
          }.errback {
      ...
      describe "deferrable callbacks" do
        ...
        it "should not trap exceptions in the client callback" do
          stub_log_service_post(api_event_collection_resource_url(api_url, "foo%20bar"), 201, api_success)
          expect {
            EM.run {
